...
```{r}
finaldata=data%>%filter(AGE>=18&AGE<66)%>%filter(UHRSWORK>29)%>%filter(INCWAGE>0)%>%filter(WKSWORK1>30)%>%filter(YRIMMIG==0|AGE -(2000-YRIMMIG)<= EDUCD_recoded +6)
```
```{r}
#combine the states of the US into one value 
...
```{r}
finaldata <- finaldata %>%
  dummy_cols(select_columns = 'DISABWRK', remove_first_dummy = TRUE) %>%
  dummy_cols(select_columns = 'REGION', remove_first_dummy = TRUE)%>%
   dummy_cols(select_columns = 'SPEAKENG', remove_first_dummy = TRUE)
...
for (country in countries) {
  ...
  if (sum(complete.cases(country_data)) > 0) {
    ...
    # Fit the model
  bigmodels[[length(bigmodels) + 1]] <- lm(log(wages) ~ EDUCD_recoded + DISABWRK+SEX+ potentialexperience^4 +REGION_21+REGION_22+REGION_12+REGION_31+REGION_32+REGION_33+REGION_41+REGION_42+ DISABWRK_4, data = country_data)
