...
```{r}
finaldata=finaldata%>%filter(AGE>=18&AGE<66)%>%filter(UHRSWORK>29)%>%filter(INCWAGE>0)%>%filter(WKSWORK1>=30)%>%filter(YRIMMIG==0|AGE -(2000-YRIMMIG)<= EDUCD_recoded +6)
```
```{r}
...
finaldata <- finaldata %>%
  ...
  dummy_cols(select_columns = 'REGION', remove_first_dummy = TRUE)%>%
   dummy_cols(select_columns = 'SPEAKENG', remove_first_dummy = TRUE)%>% dummy_cols(select_columns = 'METRO', remove_first_dummy = TRUE)%>%dummy_cols(select_columns = 'CITIZEN', remove_first_dummy = TRUE)

```
```{r}
finaldata$hsgrad=ifelse(finaldata$EDUCD_recoded>1,1,0)
```
...
for (country in countries) {
  ...
  if (sum(complete.cases(country_data)) > 0) {
    ...
    # Fit the model
  bigmodels[[length(bigmodels) + 1]] <- lm(log(wages) ~ EDUCD_recoded + DISABWRK+SEX+ potentialexperience^4 +REGION_21+REGION_22+REGION_12+REGION_31+REGION_32+REGION_33+REGION_41+REGION_42+ DISABWRK_1 + SPEAKENG_3 +hsgrad+ SPEAKENG_4+SPEAKENG_6+SPEAKENG_5+METRO_1+METRO_2+METRO_3+METRO_4+CITIZEN_1+CITIZEN_2+CITIZEN_3, data = country_data)
...
df2$samplesize <- sapply(row.names(df2), function(x) {
    idx <- which(finaldata$bplusone == x)
    if (length(idx) == 0) NA else length(idx)
})
